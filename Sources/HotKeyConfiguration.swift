...
struct HotKeyConfiguration: Equatable, Codable {
    ...
    static func from(event: NSEvent) -> HotKeyConfiguration {
        HotKeyConfiguration(
            ...
            modifiers: Int(event.modifierFlags.intersection(.deviceIndependentFlagsMask).rawValue)
        )
    }

    static let `default` = HotKeyConfiguration(keyCode: 62, modifiers: Int(NSEvent.ModifierFlags.control.rawValue))
}
